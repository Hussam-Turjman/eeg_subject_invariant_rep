...
class DataLoader:
    def __init__(self, dataset, class_combo, train_type=None, data_type=None, num_class=2, subject=None,
                 data_format=None, dataset_path='/datasets', **kwargs):

        self.class_combo = class_combo  # 'left_right', 'left_foot', 'right_foot'
        ...
        self.path = Path(self.dataset_path, self.dataset, self.data_type, self.class_combo, self.train_type)
    ...
    def load_train_set(self, fold, load_subject_ids=False, **kwargs):
        self.fold = fold
        for k in kwargs.keys():
            self.__setattr__(k, kwargs[k])
        ...
        try:
            ...
            self.file_y = self.path + '/y_train_{}{:03d}_fold{:03d}.npy'.format(self.prefix_name, self.subject,
                                                                                self.fold)
            self.file_subject_ids = self.path + '/subject_ids_train_{}{:03d}_fold{:03d}.npy'.format(self.prefix_name,
                                                                                                    self.subject,
                                                                                                    self.fold)
            X = self._change_data_format(np.load(self.file_x))
            y = np.load(self.file_y)
            if load_subject_ids:
                subject_ids = np.load(self.file_subject_ids)
        except:
            raise Exception(
    ...
    def load_val_set(self, fold, load_subject_ids=False, **kwargs):
        self.fold = fold
        for k in kwargs.keys():
            self.__setattr__(k, kwargs[k])
        ...
        try:
            ...
            self.file_subject_ids = self.path + '/subject_ids_val_{}{:03d}_fold{:03d}.npy'.format(self.prefix_name,
                                                                                                  self.subject,
                                                                                                  self.fold)
            X = self._change_data_format(np.load(self.file_x))
            y = np.load(self.file_y)
    ...
    def load_test_set(self, fold, load_subject_ids=False, **kwargs):
        self.fold = fold
        for k in kwargs.keys():
            self.__setattr__(k, kwargs[k])
        ...
        try:
            ...
            self.file_y = self.path + '/y_test_{}{:03d}_fold{:03d}.npy'.format(self.prefix_name, self.subject,
                                                                               self.fold)
            self.file_subject_ids = self.path + '/subject_ids_test_{}{:03d}_fold{:03d}.npy'.format(self.prefix_name,
                                                                                                   self.subject,
                                                                                                   self.fold)
            X = self._change_data_format(np.load(self.file_x))
            y = np.load(self.file_y)
