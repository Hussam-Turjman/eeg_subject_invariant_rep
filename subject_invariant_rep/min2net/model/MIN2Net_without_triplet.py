from tensorflow.keras.layers import Concatenate, AveragePooling2D, BatchNormalization, Conv2D, Conv2DTranspose, Dense, \
...
from ..loss import mean_squared_error, SparseCategoricalCrossentropy
from ..utils import compute_class_weight
from ..callbacks import TimeHistory
...
class MIN2Net_without_triplet:
    ...
    def build(self):
        ...
        de_conv = Conv2DTranspose(filters=self.filter_2, kernel_size=(1, 64), activation='elu', padding='same',
                                  strides=self.pool_size_2,
                                  kernel_constraint=max_norm(2., axis=(0, 1, 2)))(de_conv)
        decoder_output = Conv2DTranspose(filters=self.filter_1, kernel_size=(1, 32), activation='elu', padding='same',
                                         strides=self.pool_size_1,
                                         kernel_constraint=max_norm(2., axis=(0, 1, 2)))(de_conv)
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if X_train.ndim != 4:
            raise Exception(
                'ValueError: `X_train` is incompatible: expected ndim=4, found ndim={}'.format(X_train.ndim))
        elif X_val.ndim != 4:
