...
from .eegnet_latest import EEGNetModel
from ...models.utils import to_logits, to_labels
...
class EEGNet:
    ...
    def build(self):
        model = EEGNetModel(nb_classes=self.num_class,
                            Chans=self.Chans,
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if self.class_balancing:  # compute_class_weight if class_balancing is True
            self.class_weight = compute_class_weight(to_labels(y_train))
        else:
            self.class_weight = None
    ...
    def predict(self, X_test, y_test):
        ...
        print(classification_report(to_labels(y_test), y_pred_argm))
        print("F1-score is computed based on {}".format(self.f1_average))
        f1 = f1_score(to_labels(y_test), y_pred_argm, average=self.f1_average)
        ...
        Y = {'y_true': y_test,
             'y_pred': y_pred_argm}
        encoder = model.get_layer("flatten")
        embeddings = encoder(X_test)
        return Y, evaluation, embeddings
